...
-- Enforce that event reminders are scheduled before the event starts
CREATE OR REPLACE FUNCTION check_reminder_scheduled_before_event()
RETURNS TRIGGER AS $$
DECLARE
    event_start TIMESTAMPTZ;
BEGIN
    SELECT start_time INTO event_start FROM public.events WHERE id = NEW.event_id;
    IF NEW.scheduled_for >= event_start THEN
        RAISE EXCEPTION 'Reminder must be scheduled before event start time';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
DROP TRIGGER IF EXISTS check_reminder_before_event ON public.event_reminders;
CREATE TRIGGER check_reminder_before_event
    BEFORE INSERT OR UPDATE ON public.event_reminders
    FOR EACH ROW EXECUTE FUNCTION check_reminder_scheduled_before_event();
