-- Search Filters
-- Adds filter parameters to search_events_ranked function
-- Migration: 20260126_search_filters

-- Drop and recreate search_events_ranked function with filter parameters
DROP FUNCTION IF EXISTS public.search_events_ranked(TEXT, INT, UUID);
...
-- Add helpful comment
COMMENT ON FUNCTION public.search_events_ranked IS 'Full-text search for events with relevance ranking, typo tolerance, and filters (type, date range, size)';
